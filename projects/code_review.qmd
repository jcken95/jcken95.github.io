...
When reviewing code, it's a good idea to follow a mental checklist. I find the following works well for me and the team I'm part of:
  ...
  * Does the code have a consistent style with well-structured function and variable names?
...
We will first think about the literal act of reviewing code, and then later we will think about how a review should behave.

 

## Literal code review

* Make comments directly on the code

   * Click the `+` symbol to comment on a single line of code. Click a `+` and drag down to comment on multiple lines.
...
![image](image of + sign)
   ...
   * Click the little `+/-` symbol to make a direction suggestion on the code. This is useful when it's simply easier to make the fix, than telling someone how to make the fix

 

![image](image of adding a suggest)
   ...
   * Comments on code start a "conversation" which can be resolved. It is up to the reviewer to resolve conversations. I'd encourage authors to use emojis or comments to track their work.
...
## Guiding principles: how should the reviewer behave?
...
If you find yourself being blunt, [Owen](https://github.com/owenjonesuob) recommends having a cup of tea before finalising your review. I think he's right.
